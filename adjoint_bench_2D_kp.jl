...
cost(H, H_obs) = 0.5*sum((H.-H_obs).^2+(qHx.-qHx_pbs).^2+(qHy.-qHy_obs).^2)

function compute_D!(D, gradS, av_ya_∇Sx, av_xa_∇Sy, H, B, a, As, n, dx, dy)
    @get_thread_idx(H)
    if (ix<=size(H,1)-1 && iy<=size(H,2)-1)
        ...
        @inbounds D[ix, iy] = (a*@av_xy(H)^(n+2)+As[ix,iy]*@av_xy(H)^n)*gradS[ix,iy]^(n-1)
    end 
    return 
end 
...
function residual!(RH, qHx, qHy, β, H, B, z_ELA, b_max, dx, dy)
    @get_thread_idx(H)
    if (ix <= size(H,1)-2 && iy <= size(H,2)-2)
        @inbounds RH[ix+1, iy+1] = -(@d_xa(qHx)/dx + @d_ya(qHy)/dy) + min(β[ix+1,iy+1]*(H[ix+1, iy+1]+B[ix+1, iy+1]-z_ELA[ix+1, iy+1]), b_max)
    end 
    return 
end 

function compute_abs_error!(Err_abs, qHx, qHy, β, H, B, z_ELA, b_max, dx, dy)
    @get_thread_idx(H) 
    if (ix <= size(H,1)-2 && iy <= size(H,2)-2)
        @inbounds Err_abs[ix+1,iy+1] = - (@d_xa(qHx)/dx+@d_ya(qHy)/dy) + min(β[ix+1, iy+1]*(H[ix+1,iy+1]+B[ix+1,iy+1]-z_ELA[ix+1, iy+1]),b_max)
        if H[ix+1,iy+1] ≈ 0.0 
...
@views function forward_solve!(fields, scalars, iter_params; visu=nothing)
    (;H, B, D, gradS, av_ya_∇Sx, av_xa_∇Sy, qHx, qHy, β, As, RH, Err_rel, Err_abs, z_ELA) = fields 
    (;n, nx, ny, dx, dy, maxiter, ncheck, threads, blocks)                                = scalars
    (;a, b_max, ϵtol)                                                                     = iter_params
    ...
    for iter in 1:maxiter
        ...
        CUDA.@sync @cuda threads=threads blocks=blocks compute_D!(D, gradS, av_ya_∇Sx, av_xa_∇Sy, H, B, a, As, n, dx, dy)
        CUDA.@sync @cuda threads=threads blocks=blocks compute_q!(qHx, qHy, D, H, B, dx, dy)
        dτ=1/(12.1*maximum(D)/dx^2+maximum(β))
        CUDA.@sync @cuda threads=threads blocks=blocks residual!(RH, qHx, qHy, β, H, B, z_ELA, b_max, dx, dy)
        ...
        if iter==1 || iter %ncheck == 0
            CUDA.@sync @cuda threads=threads blocks=blocks compute_abs_error!(Err_abs, qHx, qHy, β, H, B, z_ELA, b_max, dx, dy)
            ...
            if !isnothing(visu)
                p1 = heatmap(Array(H'); title="Ice thickness H (forward model)")
...
@views function adjoint_solve!(fwd_params, adj_params, loss_params)
    #unpack forward 
    (;H, B, D, gradS, av_ya_∇Sx, av_xa_∇Sy, qHx, qHy, β, As, RH, z_ELA) = fwd_params.fields
    ...
    CUDA.@sync @cuda threads=threads blocks=blocks ∇(compute_D!,
            ...
            Const(H), 
            Const(B), Const(a), DupNN(As, dJ_dH), Const(n), Const(dx), Const(dy))
    ...
    while merr >= ϵtol_adj && iter < maxiter
        ...
        CUDA.@sync @cuda threads=threads blocks=blocks ∇(residual!,
            ...
            DupNN(qHx,q̄Hx), 
            DupNN(qHy,q̄Hy),
            Const(β), Const(H), Const(B), Const(z_ELA), Const(b_max), Const(nx), Const(ny), 
            Const(dx), Const(dy))
        #compute_q!(qHx, qHy, D, H, B, dx, dy)
        CUDA.@sync @cuda threads=threads blocks=blocks ∇(compute_q!,
            DupNN(qHx, q̄Hx), 
            ...
            DupNN(H,H̄),
            Const(B), Const(dx), Const(dy))
        #compute_D!(D, gradS, av_ya_∇Sx, av_xa_∇Sy, H, B, a, as, n, dx, dy)
        #dR .+= H̄_1 + H̄_2 + H̄_3
...
@views function loss(fwd_params, loss_params;kwags...)
    
    (;H_obs, qHx_obs, qHy_obs) = loss_params.fields
    
    @info "Forward solve"
    #forward_solve!(fields, scalars, iter_params; visu=nothing)
    forward_solve!(fwd_params.fields, fwd_params.scalars, fwd_params.iter_params)
...
function ∇loss!(Ās,fwd_params,adj_params,loss_params; reg=nothing,kwargs...)
    
    (;RH,qHx,qHy,β, H, B, z_ELA,D,gradS,av_ya_∇Sx, av_xa_∇Sy,As, ) = fwd_params.fields 
    (;dx, dy,n ) = fwd_params.scalars 
    (;b_max,a,) = fwd_params.iter_params
    (;R̄H, ψ_H, q̄Hx, q̄Hy, D̄,) = adj_params.fields
    (;∂J_∂qx, ∂J_∂qy, ) = loss_params.fields 

    @info "Forward solve"
    forward_solve!(fwd_params...; kwargs...)
    
    @info "Adjoint solve"
    adjoint_solve!(fwd_params, adj_params, loss_params)
    #solve for ψ
    ...
    CUDA.@sync @cuda threads=threads blocks=blocks ∇(residual!,
        DupNN(RH,R̄H), 
        DupNN(qHx,q̄Hx), #dR_qHx
        DupNN(qHy,q̄Hy), #dR_qHy
        Const(β), Const(H), Const(B), Const(z_ELA), Const(b_max), Const(dx), Const(dy))
    #compute_q!(qHx, qHy, D, H, B, dx, dy)
    CUDA.@sync @cuda threads=threads blocks=blocks ∇(compute_q!,
        DupNN(qHx, q̄Hx),
        DupNN(qHy,  q̄Hy),
    ...
    #convert to dj_dlogK by chain rule
    Ās .*= As #logĀs
    return 
end
...
@views function main()
    ...
    q̄Hx         = CUDA.zeros(Float64,)
    q̄Hy         = CUDA.zeros(Float64,)
    D̄           = CUDA.zeros(Float64,)
    H̄           = CUDA.zeros(Float64,)
    R̄H          = CUDA.zeros(Float64,)
    ψ_H         = CUDA.zeros(Float64,)
    qHx_obs     = CUDA.zeros(Float64,)
    qHy_obs     = CUDA.zeros(Float64,)
    dJ_dH       = CUDA.zeros(Float64,)
    ∂J_∂qx      = CUDA.zeros(Float64,)
    ∂J_∂qy      = CUDA.zeros(Float64,)
    ...
    #action 
    fwd_params = (
        fields       =     (;H,B,D,gradS,av_ya_∇Sx, av_xa_∇Sy, qHx,qHy, β, As, RH, Err_rel, Err_abs, z_ELA), 
        scalars     =     (;n, nx, ny, dx, dy, maxiter, ncheck, threads, blocks), 
        iter_params =     (;a, b_max, ϵtol),
    )
    fwd_syn_params =(
        fields       =      (;H_obs, B, D, gradS, av_ya_∇Sx, av_xa_∇Sy, qHx, qHy, β, As_syn, RH, Err_rel, Err_abs, z_ELA),
        scalars     =      (;n, nx, ny, dx, dy, maxiter, ncheck, threads, blocks),
        iter_params =      (;a, b_max, ϵtol),
    )
    #fwd_visu  = (;) forward visualization
    @info "Synthetic solve" # solve synthetic with synthetic value of as 
    forward_solve!(fwd_syn_params...; visu=fwd_visu)
    #store true data 
    ...
    reg         = (;nsm=50, Tmp)
    # loss functions 
    # loss(fwd_params, loss_params;kwags...)
    # ∇loss!(Ās,fwd_params,adj_params,loss_params; reg=nothing,kwargs...)
    loss(fwd_params, loss_params)
    ∇loss!(Ās, fwd_params, adj_params, loss_prams; reg)
