...
@paralel_indices (ix, iy) function bc!(qHx::Data.Array, qHy::Data.Array, H)
    nx, ny = size(H)
    if (ix<=nx+1 && iy==1   )  qHx[ix,iy] = qHx[ix,iy+1] end
    if (ix<=nx+1 && iy==ny  )  qHx[ix,iy] = qHx[ix,iy-1] end
    if (ix==1    && iy<=ny+1)  qHy[ix,iy] = qHy[ix+1,iy] end
...
@parallel_indices (ix,iy) function compute_1!(B, B_avg, Bx, By)
    nx, ny = size(B)
...
@parallel_indices (ix, iy) function compute_diffus!(S, H_avg, B_avg, dSdx, dSdy, gradS, D, H, n, a, as, dx, dy)
    nx,ny = size(H)
    if ix<= nx-1 && iy <= ny-1
...
@paralel_indices (ix,iy) function compute_flux!(S, qHx, qHy, D, Bx, By, dx, dy) 
    nx, ny = size(S) 
...
@parallel_indices (ix,iy) function update_H!(dHdt,H,S,B,qHx,qHy,M,dt,dx,dy)
...
function update_!(S, H, B, M, B_avg, H_avg, qHx, qHy, dSdx, dSdy, gradS, D, Bx, By, dHdt, dx, dy, dt, n, a, as, threads, blocks)
...
function sia_2D()
    ...
    while t <= ttot
        ...
        update_2!(S, H, B, M, B_avg, H_avg, qHx, qHy, dSdx, dSdy, gradS, D, Bx, By, dHdt, dx, dy, dt, n, a, as, threads, blocks)
        if it%nout == 0
