...
using ImplicitGlobalGrid # first step 
@static if USE_GPU
...
# from parallelstencil to implicitGlobalgrid.jl 
# implicitGlobalGrid.jl can render distributed paralleliation with GPU and CPU for HPC a very simple simple task
# implicitGlobalGrid.jl combines with ParallelStencil.jl 

# using both pacakges together enables to hide communication behind computation 
...
# only three functions are required to perform halo updates close to hardware limite
#
