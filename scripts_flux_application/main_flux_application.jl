...
function adjoint_2D()
    ...
    #pack parameters
    fwd_params = (fields           = (; H, H_old, B, β, ELA, D, qHx, qHy, As, RH, qmag, mask, mb, Err_rel, Err_abs),
    ...
    begin
        ...
        axs = (H_obs    = Axis(fig[1, 1][1, 1]; aspect=DataAspect(), xlabel=L"x", ylabel=L"y", title=L"H_{obs}"),
               qmag_obs  = Axis(fig[1, 2]; aspect=DataAspect(), title=L"qmag_{obs}"),
               As   = Axis(fig[1, 3][1, 1]; aspect=DataAspect(), xlabel=L"x", title=L"\log_{10}(A_s)"),
               #mb   = Axis(fig[1, 3][1, 1]; aspect=DataAspect(), xlabel=L"x", title=L"mb"),
               H    = Axis(fig[2, 1][1, 1]; aspect=DataAspect(), xlabel=L"x", ylabel=L"y", title=L"H_{modeled}"),
               qmag  = Axis(fig[2, 2][1,1]; aspect=DataAspect(), title=L"qmag_{modeled}"),
        ...
        plts = (H_obs      = heatmap!(axs.H_obs, xc, yc, H_obs_v; colormap=:turbo),
                ...
                As         = heatmap!(axs.As, xc[1:end-1], yc[1:end-1], As_v; colormap=:turbo), 
                H          = heatmap!(axs.H, xc, yc, H_v; colormap=:turbo),
                qmag       = heatmap!(axs.qmag, xc[2:end-1], yc[2:end-1], qmag_v; colormap=:turbo, colorrange=(0.0, 1000.0)),
                conv  = scatterlines!(axs.conv, Point2.(iter_evo, cost_evo); linewidth=2))


        Colorbar(fig[1, 1][1, 2], plts.H_obs)
