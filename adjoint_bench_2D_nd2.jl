...
mutable struct AdjointProblem{T<:Real, A<:AbstractArray{T}}
    ...
    z_ELA::A; b_max::T; nx::Int; ny::Int; dx::T; dy::T; a::T; n::Int; dmp::T; ϵtol::T; niter::Int; ncheck::Int; threads::Tuple{Int, Int}; blocks::Tuple{Int, Int}
end 
...
function solve!(problem::AdjointProblem) 
    ...
    dt = 1.0/(8.1*maximum(D)/dx^2 + maximum(β))/5000.0
    ...
    while merr >= ϵtol  && iter < niter 
        ...
        if iter % ncheck == 0 
            ...
            p3 = heatmap(xc, yc, Array(dR'); title="dR")
...
function adjoint_2D()
    ...
    β        =  3.1709791983764586e-10#1.0 # length scale lx = 1e3 (natural value)
    aρgn0    =  1.3517139631340713e-12 #1.0 # A*(ρg)^n # = 1.3475844936008e-12 (natural value)
    #scales 
    tsc      =  1/β # also calculated from natural values tsc = 3.1536e9
    lsc      = (1/aρgn0/tsc)^(1/n) # characteristic length scale lsc = 6.173703896956485
    #non-dimensional numbers (calculated from natural values)
    lx_l     =  40494.32952611237 #horizontal length to characteristic length ratio
    ly_l     =  32395.4636208899 #horizontal length to characteristic length ratio 
    lz_l     =  161.9773181044495  #vertical length to charactertistic length ratio
    B0_l     =  566.9206133655732 # maximum bed rock elevation to characteristic length ratio
    s_f_syn  =  789.5114953176602 # sliding to ice flow ratio: s_f_syn = asρgn0_syn/aρgn0/lx^2
    s_f      =  69255.39432611053 # sliding to ice flow ratio: s_f   = asρgn0/aρgn0/lx^2
    z_ela_l  =  348.2512339245664 # ela to domain length ratio z_ela_l = 
    m_max_nd =  16.19773181044495 #m_max/lx*tsc m_max = 2.0 m/a lx = 1e3 tsc = 
    βtsc     =  1.0 #ratio between characteristic time scales of ice flow and accumulation/ablation βtsc = β0*tsc 
    ...
    m_max       = m_max_nd*lsc/tsc  #2.0 m/a = 6.341958396752917e-8 m/s
    ...
    β   .+= 0.015/3600/24/365 .*atan.(xc./lx)
    ela .+= 900 .*atan.(yc'./ly .+ 0 .*xc)

    H_obs = copy(H)
    H_ini = copy(H)
    S_obs = copy(S)

    B = @. B0*(exp(-xc^2/1e10 - yc'^2/1e9) + exp(-xc^2/1e9-(yc'-ly/8)^2/1e10))
    ...
    adjoint_problem = AdjointProblem(H, H_obs,B, D, qHx, qHy, β, as, gradS, av_ya_∇Sx, av_xa_∇Sy, ela, m_max, nx, ny, dx, dy, aρgn0, n, dmp_adj, ϵtol_adj, maxiter, ncheck_adj, threads, blocks)
