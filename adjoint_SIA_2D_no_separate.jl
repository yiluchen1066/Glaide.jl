...
#function compute_∇S!(∇Sx, ∇Sy, H, B, dx, dy, nx, ny)  
#    @get_thread_idx(H) 
#    if ix <= nx-1 && iy <= ny 
#        ∇Sx[ix,iy]=(H[ix+1, iy]+B[ix+1,iy]-H[ix,iy]-B[ix,iy])/dx 
#    end 
#    if ix <= nx && iy <= ny -1 
#        ∇Sy[ix,iy]=(H[ix, iy+1]+B[ix,iy+1]-H[ix,iy]-B[ix,iy])/dy 
#    end 
#    return
#end 

#function compute_D_2!(D,gradS, ∇Sx, ∇Sy, H, a, n, as, nx, ny, dx, dy)
#    @get_thread_idx(H)
#    if ix<=nx-1 && iy<= ny-1 
...
function compute_qHx!(qHx, H, D, B, dx, dy, nx, ny)
    @get_thread_idx(H)
    if ix <= nx -1 && iy <= ny-2 
...
function compute_qHy!(qHy, H, D, B, dx, dy, nx, ny) 
    @get_thread_idx(H) 
    if ix <= nx-2 && iy <= ny-1 
...
mutable struct Forwardproblem{T<:Real, A<:AbstractArray{T}}
    H::A; B::A; D::A; gradS::A; av_ya_∇Sx::A; av_xa_∇Sy::A; qHx::A; qHy::A; β::A; as::A; ∇Sx::A; ∇Sy::A 
...
function Forwardproblem(H, B, D, gradS, av_ya_∇Sx, av_xa_∇Sy, qHx, qHy, β, as, ∇Sx, ∇Sy, n, a, b_max, z_ELA, dx, dy, nx, ny, epsi, cfl, ϵtol, niter, ncheck, threads, blocks, dmp)
    RH = similar(H,nx, ny)
    dHdτ = similar(H, nx-2, ny-2) 
    dτ   = similar(H, nx-2, ny-2) 
    Err  = similar(H, nx, ny)
    return Forwardproblem(H, B, D, gradS, av_ya_∇Sx, av_xa_∇Sy, qHx, qHy, β, as,∇Sx, ∇Sy, dτ, dHdτ, RH, Err, n, a, b_max, z_ELA, dx, dy, nx, ny, epsi, cfl, ϵtol, niter, ncheck, threads, blocks, dmp) 
end 

#solve for H using pseudo-trasient method 
function solve!(problem::Forwardproblem)
    (;H, B, D, gradS, av_ya_∇Sx, av_xa_∇Sy, qHx, qHy, β, as, ∇Sx, ∇Sy, dτ, dHdτ,RH, Err, n, a, b_max, z_ELA, dx,dy, nx, ny, epsi, cfl, ϵtol, niter, ncheck, threads, blocks, dmp) = problem
    ...
    while merr >= ϵtol && iter < niter 
        ...
        #CUDA.@sync @cuda threads=threads blocks=blocks compute_∇S!(∇Sx, ∇Sy, H, B, dx, dy, nx, ny)  
        #CUDA.@sync @cuda threads=threads blocks=blocks compute_D_2!(D,gradS, ∇Sx, ∇Sy, H, a, n, as, nx, ny, dx, dy)
        #CUDA.@sync @cuda threads=threads blocks=blocks compute_q!(qHx, qHy, D, H, B, nx, ny, dx, dy)
        CUDA.@sync @cuda threads=threads blocks=blocks compute_qHx!(qHx, H, D, B, dx, dy, nx, ny)
        CUDA.@sync @cuda threads=threads blocks=blocks compute_qHy!(qHy, H, D, B, dx, dy, nx, ny) 
...
#compute_qHx!(qHx, H, D, B, dx, dy, nx, ny)
function grad_residual_H_2!(tmp3, dR_qHx, H, dqHx_H, D, dqHx_D, B, dx, dy, nx, ny)
    Enzyme.autodiff_deferred(compute_qHx!, Duplicated(tmp3, dR_qHx), Duplicated(H, dqHx_H), Duplicated(D, dqHx_D), Const(B), Const(dx), Const(dy), Const(nx), Const(ny))
    return 
end 

#compute_qHy!(qHy, H, D, B, dx, dy, nx, ny)
function grad_residual_H_3!(tmp4, dR_qHy, H, dqHy_H, D, dqHy_D, B, dx, dy, nx, ny)
    Enzyme.autodiff_deferred(compute_qHy!, Duplicated(tmp4, dR_qHy), Duplicated(H, dqHy_H), Duplicated(D, dqHy_D), Const(B), Const(dx), Const(dy), Const(nx), Const(ny))
    return 
end 

function grad_residual_H_4!(dq_D, H, dqHx_D, dqHy_D, nx, ny)
    @get_thread_idx(H)
    if ix <= nx-1 && iy <= ny-1 
            dq_D[ix,iy] = dqHx_D[ix,iy] + dqHy_D[ix,iy]
    end 
    return 
end 
...
function grad_residual_H!(dR, dD_H, dqHx_H, dqHy_H, dR_H, nx, ny)
    @get_thread_idx(dR)
    if ix <= nx && iy <= ny 
        dR[ix,iy] = dD_H[ix,iy] + dqHx_H[ix,iy] + dqHy_H[ix,iy] + dR_H[ix,iy]
    end
    return 
end 
...
function AdjointProblem(H, H_obs,B, D, qHx, qHy, β, as, gradS, av_ya_∇Sx, av_xa_∇Sy, z_ELA, b_max, nx, ny, dx, dy, a, n, dmp, ϵtol, niter, ncheck, threads, blocks)
    ...
    return AdjointProblem(H, H_obs,B, D, qHx, qHy, β, as, gradS, av_ya_∇Sx, av_xa_∇Sy, r, dR, R, Err, dR_qHx, dR_qHy, dR_H, dqHx_H, dqHy_H, dqHx_D, dqHy_D, dq_D, dD_H, tmp1, tmp2, tmp3, tmp4, tmp5, ∂J_∂H, z_ELA, b_max, nx, ny, dx, dy, a, n, dmp, ϵtol, niter, ncheck, threads, blocks)
end 


# solve for r using pseudo-trasient method to compute sensitvity afterwards 
function solve!(problem::AdjointProblem) 
    (; H, H_obs, B, D, qHx, qHy, β, as, gradS, av_ya_∇Sx, av_xa_∇Sy, r, dR, R, Err, dR_qHx, dR_qHy, dR_H, dqHx_H, dqHy_H, dqHx_D, dqHy_D, dq_D, dD_H, tmp1, tmp2, tmp3, tmp4, tmp5, ∂J_∂H, z_ELA, b_max, nx, ny, dx, dy, a, n, dmp, ϵtol, niter, ncheck, threads, blocks) = problem
    r.= 0;  R.= 0; Err .= 0; dR .= 0; dR_qHx .= 0; dR_qHy .= 0; dR_H .= 0; dqHx_H .=0; dqHy_H .= 0; dqHx_D .= 0; dqHy_D.= 0; dq_D .= 0; dD_H .= 0
    lx, ly = 30e3, 30e3
    dx = lx/nx
    dy = ly/ny
    xc = LinRange(dx/2, lx-dx/2, nx) 
    ...
    while merr >= ϵtol  && iter < niter 
        #Err .= r 
        # compute dQ/dH 
        dR_qHx .= 0; dR_qHy .= 0; dR_H .= 0; dqHx_H .= 0; dqHy_H .= 0; dqHx_D .= 0; dqHy_D .= 0; dq_D .= 0; dD_H .= 0 
        dR .= .-∂J_∂H; tmp2 .= r 
...
function adjoint_1D()
    ...
    #Forwardproblem(H, B, D, gradS, av_ya_∇Sx, av_xa_∇Sy, qHx, qHy, β, as, ∇Sx, ∇Sy, n, a, b_max, z_ELA, dx, dy, nx, ny, epsi, cfl, ϵtol, niter, ncheck, threads, blocks, dmp)
    synthetic_problem = Forwardproblem(H_obs, B, D, gradS, av_ya_∇Sx, av_xa_∇Sy, qHx, qHy, β,as_syn, ∇Sx, ∇Sy, n, a, b_max, z_ELA, dx, dy, nx, ny, epsi, cfl, ϵtol, niter, ncheck, threads, blocks, dmp)
    forward_problem = Forwardproblem(H, B, D, gradS, av_ya_∇Sx, av_xa_∇Sy, qHx, qHy, β,as, ∇Sx, ∇Sy, n, a, b_max, z_ELA, dx, dy, nx, ny, epsi, cfl, ϵtol, niter, ncheck, threads, blocks, dmp)
