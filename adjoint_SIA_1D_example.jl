...
function flux_q!(S, H, D, ∇Sx, qHx, nx, dx, a, as, n)
    @get_thread_idx(S)
    if ix <= nx-1 
        ∇Sx[ix] = @d_xa(S)/dx 
...
function residual!(S, H, D, ∇Sx, qHx, M, RH, n, a, as, dx, nx) 
    @get_thread_idx(S)
    CUDA.@sync @cuda threads=threads blocks=blocks flux_q!(S, H, D, ∇Sx, qHx, nx, dx, a, as, n)
    if ix <= nx-2 
...
function residual_grad!()
    tmp2 .= dQ
    # dq/dH
    Enzyme.autodiff_deferred(flux_q!, Duplicated(tmp1, tmp2), Duplicated(H, dQ), Const(S), Const(D), Const(∇Sx), Const(dx), Const(a), Const(as), Const(n))
    # dR/dq
    Enzyme.autodiff_deferred(residual!, Duplicated(tmp1, tmp2), Duplicated())
